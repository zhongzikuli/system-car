...
$(function(){
	...
	$(".reset-btn").on("click", function(){
		$("#search-audit-status, #search-select-department, #keyword, #search-start-date, #search-end-date").val("");
	...
	$(".audit-btn").on("click", function(){
		...
		$.ajax({
			url: ctx + "/cfMortgageRegist/getSeat.action",
			type: "post",
			data: {
				id:applyId
			},
			...
			success: function (data) {
				if (data.error == 1) {
					if(data["rows"]){
						var url = ctx + "/cfMortgageRegist/preAudit.action?id="+ applyId;
						openTabForParent(url, "-transfer-audit-" + orderId, "抵押登记审核-" + dataTitle);
					}else{
						faildMsg("该记录已占位不能审核");
					}
					...
					success	: function (data) {
						if (data.error == 1) {
							successMsg("操作成功！", 1000, function () {
								window.location.href = ctx + "/cfMortgageRegist/queryForAudit.action";
							});
						} else if (data.error == -100) {
							faildMsg("会话超时，请重新登陆！");
						} else {
