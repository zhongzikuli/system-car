...
$(function(){
	//搜索时间控件
	var start = {
		elem: '#audit-car-search-start-date',
	...
	var end = {
		elem	: '#audit-car-search-end-date',
		//format	: 'YYYY-MM-DD hh:mm:ss',
		format: 'YYYY-MM-DD',
		min		: "1970-01-01",
		max		: laydate.now(),
	...
	//下拉框初始化
	$("#search-audit-car-status").chosen({
		disable_search_threshold	: 8,
		no_results_text				: "没有找到",
		allow_single_deselect		: true,
		width: "100%"
	...
	//刷新按钮
	$(".audit-car-refresh-btn").on("click", function(){
		$(".audit-car-search-btn").trigger("click");
	});
	//重置按钮
	$(".audit-car-reset-btn").on("click", function(){
		$("#search-audit-car-status").val("").trigger('chosen:updated');
		$("#search-select-car-department").val("").trigger('chosen:updated');
		$("#keyword").val("");
		$("#audit-car-search-start-date").val("");
		$("#audit-car-search-end-date").val("");
	});
	
	//审核按钮
	$(".audit-car-btn").on("click", function(){
		var _this = this;
		var applyId = $(_this).attr("data-id");
		var orderId = $(_this).attr("data-order");
		var dataTitle = $(_this).attr("data-title");
		var url = ctx + "/carInfo/preAudit.action?id="+ applyId;
		
		openTabForParent(url, "-car-audit-" + orderId, "提车资料审核-" + dataTitle);
		...
		/*$.ajax({
			...
			success: function (data) {
				if (data.error == 1) {
					if(data["rows"]){
						window.location.href = ctx + "/carInfo/preAudit.action?id="+ applyId;
					}else{
						faildMsg("该记录已占位不能审核");
					}
	...
	//换审核
	$(".change-audit-car-btn").on("click", function(){
					...
					success	: function (data) {
						if (data.error == 1) {
							successMsg("操作成功！", 1000, function () {
								window.location.href = ctx + "/carInfo/queryForAudit.action";
							});
						} else if (data.error == -100) {
							faildMsg("会话超时，请重新登陆！");
						} else {
