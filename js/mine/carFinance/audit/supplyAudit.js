...
jQuery(function ($) {
	...
	if($("#auditContent").length > 0){
		var isOld = $("#audit-transfer-file-list").length > 0 ? 1 : 0;	//是否二手车判断
		...
		if(null != auditContent["list"]){
			...
			for (var i = 0; i < auditContent["list"].length; i++) {
				...
				var fileType = auditContent["list"][i]["fileType"];
				
				var extStart = fileName.lastIndexOf(".");
				var ext = fileName.substring(extStart,fileName.length).toUpperCase();
				
				if(fileType == 4){
					supplyHtml += '<div class="col-sm-2"><div class="contact-box text-center mailbox-content">';
					if(ext!= ".BMP" && ext!= ".PNG" && ext!= ".JPG" && ext!= ".JPEG"){
						supplyHtml += '<img src="'+ctx +"/styles/images/zip.png"+'" alt="" title="'+fileName+'">';
					}else{
						supplyHtml += '<img class="pre-img" src="'+fPath+auditContent["list"][i]["fileGroup"]+'/'+auditContent["list"][i]["filePath"]+'" alt="">';
					}
					supplyHtml += '</div></div>';	
				}else if(fileType == 7){
					transferHtml += '<div class="col-sm-2"><div class="contact-box text-center mailbox-content">';
					if(ext!= ".BMP" && ext!= ".PNG" && ext!= ".JPG" && ext!= ".JPEG"){
						transferHtml += '<img src="'+ctx +"/styles/images/zip.png"+'" alt="" title="'+fileName+'">';
					}else{
						transferHtml += '<img class="pre-img" src="'+fPath+auditContent["list"][i]["fileGroup"]+'/'+auditContent["list"][i]["filePath"]+'" alt="">';
					}
					transferHtml += '</div></div>';	
				}
			}
			$("#audit-supply-file-list").html(supplyHtml);
			if(isOld){
				$("#audit-transfer-file-list").html(transferHtml);
			}
			//展示图片放大
			$("#audit-supply-file-list, #audit-transfer-file-list").find(".pre-img").on("click",function(){
				$.openPhotoGallery(this)
			});
		}
		$("#audit-supply-file-list, #audit-transfer-file-list").find(".contact-box").each(function(){
	...
	$(".back-btn").add(".agree-btn").on("click", function(){
		...
		confirmDialog(tip, function () {
			...
			$.ajax({
				url		: ctx + "/cfSupplyMaterial/audit.action",
				type	: "post",
				data	: params,
				dataType: "json",
				success	: function (data) {
	...
	//加载数据
	dataProxy.getAuditHistoryByAcceptId(acceptId, true, "#audit-detail-list", function(data){
		//初始化表格
	...
	function downLoadFile(id, fileType){
		$('<form action="'+ ctx + '/cfSupplyMaterial/downLoadeWaitingFile.action" method="POST">' +  // action请求路径及推送方法
