$(document).ready(function () {
    ...
    function getRecord(type, departmentName) {
        $.ajax({
            url: ctx + "/graphicReport/registerDeliverRecord.action",
            type: "post",
            data: {
                type: type,
    ...
    function initRateChart(yValue, dThirtyDay, dThirtyDayTitle, dFortyFiveDay, dFortyFiveDayTitle, dSixtyDay, dSixtyDayTitle, dGreaterSixtyDay, dGreaterSixtyDayTitle) {
        ...
        option = {
            ...
            legend: {
                ...
                data: ['30天未抵押', '30-45天未抵押', '45-60天未抵押', '超过60天抵押']
            },
            xAxis: [{
                show: false,
                type: 'value'
        ...
        ratio.on('click', function (param) {
            ...
            if (param.seriesType == 'bar') {
                var url = ctx + "/graphicReport/registerDeliverList.action?queryType=" + param.seriesName + "&departmentName=" + departmentName + "&name=" + param.name.substring(0, param.name.indexOf('(')) + "&type=" + type;
                ...
                var url = ctx + "/graphicReport/registerDeliver.action?departmentName=" + name;
                openTabForParent(url, "-register-deliver", name + "未抵押周期");
            }
        });
