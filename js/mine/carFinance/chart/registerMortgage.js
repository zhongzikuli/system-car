$(document).ready(function () {
    ...
    function getRecord(type, departmentName) {
        $.ajax({
            url: ctx + "/graphicReport/getRegisterMortgage.action",
            type: "post",
            data: {
                groupType: type,
    ...
    function initRateChart(yValue, dThirtyDay, dThirtyDayTitle, dFortyFiveDay, dFortyFiveDayTitle, dSixtyDay, dSixtyDayTitle, dGreaterSixtyDay, dGreaterSixtyDayTitle) {
        ...
        option = {
            ...
            legend: {
                ...
                data: ['抵押资料已寄出超时', '抵押资料已寄出未超时', '抵押资料未寄出超时', '抵押资料未寄出未超时']
            },
            xAxis: [{
                show: false,
                type: 'value'
        ...
        ratio.on('click', function (param) {
            ...
            if (param.seriesType == 'bar') {
                var url = ctx + "/graphicReport/getRegisterMortgageList.action?queryType=" + param.seriesName + "&departmentName=" + departmentName + "&name=" + param.name.substring(0, param.name.indexOf('(')) + "&type=" + type;
                ...
                var url = ctx + "/graphicReport/registerMortgage.action?departmentName=" + name;
                openTabForParent(url, "-register-deliver", name + "抵押材料");
            }
        });
