            ...
            callback: function () {
                ...
                if (fileList < 1) {
                    ...
                    if ($("#hidden-select").is(':hidden')) {
                        ...
                        param.list = fileList;
                        param.fileChildType = 40;
                    }
                    if ($("#fileCreateForm").valid("fileCreateForm")) {
                        loadingShow();
                        $.ajax({
                            url: ctx + "/evaluationReportFile/uploadReport.action",
                            type: "post",
                            ...
                            success: function (data) {
                                ...
                                if (data.error == 1) {
                                    successMsg("操作成功！", 1000, function () {
                                        window.location.href = ctx + "/evaluationReportFile/preUploadReport.action?id=" + aid + "&goBackUrl=" + $("#goBackUrl").val()+'&viewSource=' + $("#viewSource").val();
                                    });
...
function commit(acceptId) {
    confirmDialog("确认提交评估报告吗？", function () {
        ...
        if (length <= 0) {
            ...
            loadingShow();
            $.ajax({
                url: ctx+"/evaluationReportFile/reportFileSubmit.action",
                type: "post",
                data: {
                    "orderId": acceptId
                },
                    ...
                    if (data.error == 1) {
                        successMsg("操作成功！", 1000, function () {
                            ...
                            if ($("#goBackUrl").val() != null && $("#goBackUrl").val() != '') {
                                window.location.href = ctx + "/evaluationReportFile/query.action";
                            }
...
function deleteFile(title, type) {
    ...
    if (ck.length == 0) {
        ...
        confirmDialog("确认删除选中的文件吗？", function () {
            ...
            loadingShow();
            $.ajax({
                url: ctx + "/evaluationReportFile/deleteReport.action",
                type: "post",
                data: params,
                dataType: "json",
                success: function (data) {
                    ...
                    if (data.error == 1) {
                        successMsg("操作成功！", 1000, function () {
                            window.location.href = ctx + "/evaluationReportFile/preUploadReport.action?id=" + $("#OrderAcceptId").val() + "&goBackUrl=" + $("#goBackUrl").val();
                            ;
                        });
                    } else if (data.error == -100) {
                        faildMsg("会话超时，请重新登陆！");
...
function downloadFile(title, type) {
    ...
    confirmDialog("确认下载订单相关文件吗？", function () {
        window.location.href = ctx + "/evaluationReportFile/reportDownload.action?fileType=" + fileType + "&aId=" + aid;
    })
}
