(function($){
	...
	LoanCreditQuery.prototype.createQueryCategory = function (rows){
		...
		if(rows.length > 0){
			for(var i=0; i<rows.length; i++){
				...
				html += '<div class="title"><span class="name">'+ rows[i]["userTypeName"] +":"+rows[i]["realName"]+'</span> '+ (rows[i]["isBlack"] > 0 ? '<span class="pull-right badge badge-danger">已拉黑</span>' : "") +'</div>';
				var policyType = "--";
	...
	function createFrequencyDistinctHtml(title, dimType, dimValue, data){
        ...
        var list = data["list"];
        html += '<tr>';
        html += "<td width='12.5%'>命中属性字典</td><td width='12.5%'>"+getValue(HYCarFinance.TDConfig.getDimTypeName(dimType))+"</td>";
	...
	function createDiscreditCountHtml(title, data){
        ...
        if (null == hits || hits.length <= 0) {
        	...
        	html += '<td width="12.5%" class="cel text-center">字段</td><td>'+getValue(HYCarFinance.TDConfig.getDimTypeName(calcDimType))+'</td>';
	...
	function createAssociationHtml(title, data){
        ...
        if (null == hits || hits.length <= 0) {
        	...
        	for (var i = 0; i < resultsDim.length; i++) {
                ...
                html += HYCarFinance.TDConfig.getDimTypeName(dimType) + "(" + resultsDim[i]["count"] + '个)</br>';
            }
        	html += '</td>';
        	...
        	if(null != hitsDim && typeof(hitsDim) != "undefined"){
            	...
            	for (var i = 0; i < hitsDim.length; i++) {
                    ...
                    html += hitsDim[i]["industryDisplayName"] + ":" + hitsDim[i]["count"] + '('+ HYCarFinance.TDConfig.getDimTypeName(dimType) +')' + '</br>';
                }
            	html += '</td>';
        	}else{
	...
	function createCustomListHtml(title, dimType, dimValue, data){
        ...
        if (null == hits || hits.length <= 0) {
			...
			html += "<tr class='white-bg'>"
			html += "<td width='16%'>命中属性</td><td width='17%'><code class='alert-success'>"+ getValue(HYCarFinance.TDConfig.getDimTypeName(data["dimType"])) +"</code></td>";
	...
	function createBlackListHtml(title, data){
        ...
        if (null == hits || hits.length <= 0) {
        	...
        	for (var i = 0; i < hits.length; i++) {
    			...
    			html += "<tr>"
				html += "<td width='16%'>命中属性</td><td width='17%'><code class='alert-success'>"+ HYCarFinance.TDConfig.getDimTypeName(dimType) +"</code></td>";
