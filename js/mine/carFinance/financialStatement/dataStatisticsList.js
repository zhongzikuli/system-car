$(function(){
	...
	//下拉框初始化
	$("#search-moneyPackageState").chosen({
		disable_search_threshold	: 8,
		no_results_text				: "没有找到",
		allow_single_deselect		: true,
		width: "100%"
	...
	$(".reset-btn").on("click", function(){
		...
		$("#search-moneyPackage-Id").val("").trigger('chosen:updated');
		$("#search-moneyPackageState").val("").trigger('chosen:updated');
		$("#search-start-date").val("");
		$("#search-end-date").val("");
		$("#search-end-date").val("");
		$("#search-start-paymentDatestartDate").val("");
        $("#search-end-paymentDatesendDate").val("");
		$("#search-keyword").val("")
		$("#keyword").val("")
	});
	...
	initLaydate("search-start-paymentDatestartDate","search-end-paymentDatesendDate");
	//搜索时间控件
...
function creditNotDownExcel() {
	...
	confirmDialog("您确定要导出记录？", function () {
		var frame = $('<iframe>');//定义一个iframe
		frame.attr("src", ctx + "/creditNotGiveBank/exportList.action?ids=" + ids.toString());
		frame.attr("style", "display:none");
		frame.append("</iframe>")
		$("body").append(frame);
...
function exporIncomeExcel() {
	...
	confirmDialog("您确定要导出记录？", function () {
		var frame = $('<iframe>');//定义一个iframe
		frame.attr("src", ctx + "/financialIncomeStatisticalReport/exportList.action?idArr=" + idArr.toString());
		frame.attr("style", "display:none");
		frame.append("</iframe>")
		$("body").append(frame);
...
function exporChargesDetailsExcel() {
	...
	confirmDialog("您确定要导出记录？", function () {
		var frame = $('<iframe>');//定义一个iframe
		frame.attr("src", ctx + "/financialChargesDetailsReport/exportList.action?idArr=" + idArr.toString());
		frame.attr("style", "display:none");
		frame.append("</iframe>")
		$("body").append(frame);
...
function exporDateDetailsExcel() {
	...
	confirmDialog("您确定要导出记录？", function () {
		var frame = $('<iframe>');//定义一个iframe
		frame.attr("src", ctx + "/financialDataStatisticalReport/exportList.action?idArr=" + idArr.toString());
		frame.attr("style", "display:none");
		frame.append("</iframe>")
		$("body").append(frame);
