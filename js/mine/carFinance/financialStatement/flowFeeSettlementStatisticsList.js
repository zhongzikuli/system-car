$(function(){
	...
	$(".reset-btn").on("click", function(){
		$("#search-superOrgId").val("").trigger('chosen:updated');
...
function exporSettlementExcel() {
	...
	confirmDialog("您确定要导出记录？", function () {
		...
		frame.attr("src", ctx + "/flowFeeSettlementStatisticsReport/exportList.action?idArr=" + idArr.toString());
...
function exporCostExcel() {
	...
	confirmDialog("您确定要导出记录？", function () {
		var frame = $('<iframe>');//定义一个iframe
		frame.attr("src", ctx + "/flowFeeCostStatisticsReport/exportList.action?idArr=" + idArr.toString());
		frame.attr("style", "display:none");
		frame.append("</iframe>")
		$("body").append(frame);
