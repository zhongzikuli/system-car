	...
	$(".audit-btn").on("click", function(){
		...
		var options = {
			...
			onBeforeShow:function(){
				...
				$("#real-name").val(name);
				$.ajax({
					url: ctx + "/applyPay/getAuditAndAudit.action",
					type: "post",
					data: {
					...
					success: function (data) {
						if (data.error == 1) {
							if(null != data["rows"]) {
                                ...
                                if (null != data["rows"]["firstAuditTimeStr"]) {
                                    $("#audit-time").val(data["rows"]["firstAuditTimeStr"]);
                                }
                                if (null != data["rows"]["auditName"]) {
                                    $("#audit-name").val(data["rows"]["auditName"]);
                                }
                                if (null != data["rows"]["bankAccountName"]) {
                                    $("#bankAccountName").val(data["rows"]["bankAccountName"]);
                                }
                                if (null != data["rows"]["bankName"]) {
                                    $("#bankName").val(data["rows"]["bankName"]);
                                }
                                if (null != data["rows"]["bankAccount"]) {
                                    $("#bankAccount").val(data["rows"]["bankAccount"]);
                                }
                                if (null != data["rows"]["bankAccount"]) {
                                    $("#bankAccount").val(data["rows"]["bankAccount"]);
                                }
                                if(null !=  data["rows"]["actualLoadMoney"]){
                                    $("#actualLoadMoney").val(NumberFormatUtil.fmoney(data["rows"]["actualLoadMoney"], 2));
                                }
                                if(null !=  data["rows"]["businessTeamAdvanced"]){
                                    $("#businessTeamAdvanced").val( data["rows"]["businessTeamAdvanced"] ==1? 是: 否);
                                }else{

                                    $("#businessTeamAdvanced").val("否");
                                }
                                if(null !=  data["rows"]["businessTeamAdvanced"]){
                                    if(data["rows"]["businessTeamAdvanced"]=='1'){
                                        var ext = data["rows"]["fileName"].substring(extStart, fileName.length).toUpperCase();
                                       var  itemHtml = '<img src="' + ctx + "/styles/images/zip.png" + '" alt="">';
                                        ...
                                        itemHtml += '<div class="file-name">' + data["rows"]["fileName"] + '</div><i class="fa fa-times-circle" data-id="' + data["rows"]["filePath"] + '"></i></div></div>';
                                        $("#department-money-file").append(itemHtml);
                                        $("#department-money-file").show();
                                    }
                                }
                            }
			...
			callback:function(event){
				...
				if($("#applyPayForm").valid("applyPayForm")){
					loadingShow();
					$.ajax({
						url: ctx + "/applyPay/finalPrincipalAudit.action",
						type: "post",
						data: {
							id		: $("#applyId").val(),
							status	: status,
