$(function(){
	...
	$(".detail").on("click", function () {
		var url =ctx +"/hasAdvances/list.action";
	...
	//公司垫款明细导出
	$(".advances-excel").on("click", function () {
		panyMentExcel();
	});
...
function panyMentExcel() {
	...
	confirmDialog("您确定要导出记录？", function () {
		...
		frame.attr("src", ctx + "/offLoan/newExportList.action?ids=" + ids.toString());
...
function panyMentDownExcel() {
	...
	confirmDialog("您确定要导出记录？", function () {
		var frame = $('<iframe>');//定义一个iframe
		frame.attr("src", ctx + "/hasAdvances/newExportList.action?ids=" + ids.toString());
		frame.attr("style", "display:none");
		frame.append("</iframe>")
		$("body").append(frame);
