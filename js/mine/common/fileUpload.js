(function ($) {
    ...
    function initComponentData(options) {
        if (options.initData && options.initData.length > 0) {
            ...
            for (var i = 0; i < arrData.length; i++) {
                ...
                if(ext!= ".BMP" && ext!= ".PNG" && ext!= ".JPG" && ext!= ".JPEG"){
                	var $li = $('<li class="state-complete" id="E_FILE_' + obj.fileId + '" data-group="' + obj.fileGroup + '" data-filepath="' + obj.filePath + '" data-fileName="' + obj.fileName + '">' +
                            '<p class="title">' + obj.fileName + '</p>' +
                            '<p class="imgWrap"><img id="vtipArrow" src="'+ctx +"/styles/images/zip.png"+'"></p>'+
        ...
        cancel.on("click", function () {
            ...
            return removeFile2(options,id);
//        	var group = $(this).parents(".state-complete").attr("data-group");
//            var filePath = $(this).parents(".state-complete").attr("data-filepath");
...
//            }
    ...
    function addFile(uploader, file) {
        ...
        uploader.makeThumb(file, function (error, ret) {
            if (error) {
                $img.append('<img id="vtipArrow" src="'+ctx +"/styles/images/zip.png"+'"/>');
            } else {
    ...
    //编辑删除时用到
    function removeFile2(option,liId) {
