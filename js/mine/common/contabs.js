$(function() {
    ...
    function c() {
        ...
        $(".J_menuTab").each(function() {
            if ($(this).data("id") == o) {
                if (!$(this).hasClass("active")) {
                    $(this).addClass("active").siblings(".J_menuTab").removeClass("active").find("i").attr("class","fa fa-times-circle");
        ...
        loadingShow();
        if (k) {
            var p = '<a href="javascript:;" class="active J_menuTab" data-id="' + o + '">' + l + ' <i class="fa fa-times-circle-active"></i></a>';
            var n = '<iframe class="J_iframe" name="iframe' + m + '" width="100%" height="100%" src="'+ ctx + o + '" frameborder="0" data-id="' + o + '" seamless></iframe>';
            $(".J_menuTab").removeClass("active").find("i").attr("class","fa fa-times-circle");
            if(  $(".J_mainContent").find("iframe.J_iframe").hide().parents(".J_mainContent").length > 0){
            ...
            g($(".J_menuTab.active"));
            $('.J_iframe[name="iframe' + m  + '"]').load(function () {
                loadingHide();	 //关闭loading提示
            });
        }else{
            $('.J_iframe[name="iframe' + m  + '"]').attr('src', ctx + o).load(function () {
                loadingHide();	 //关闭loading提示
            });
        }
        return false
    ...
    function h() {
        ...
        if ($(this).parents(".J_menuTab").hasClass("active")) {
            ...
            if ($(this).parents(".J_menuTab").prev(".J_menuTab").size()) {
                ...
                $(this).parents(".J_menuTab").prev(".J_menuTab:last").addClass("active").find("i").attr("class","fa fa-times-circle-active");
                $(".J_mainContent .J_iframe").each(function() {
                    if ($(this).data("id") == k) {
                        $(this).show().siblings(".J_iframe").hide();
                        return false
