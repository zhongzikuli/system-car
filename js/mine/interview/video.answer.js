(function($){
	...
	function showBesideBox(){
		...
		$(parent.document.body).add($(".mod_basic, .mod_header")).on("click", function (e) {
			if(e.target.id == "viewInterviewQuestion"){
				return;
			}
			if($(".beside-query-box").hasClass("active")){
				hideBesideBox();
	...
	VideoAnswer.prototype.createQuestionList = function (rows){
		...
		$(".ask-submit").on("click", submitAsk);
	}
	
	function submitAsk(_this){
		var isvalid = true;
		var param="";
		$(".questions").find(".item").each(function(i, n){
			var dataId = $(n).attr("data-id");
			var name =$(n).attr("data-name");
			var value = $(n).find('input[name="'+name+'"]:checked ').val();
			if($(n).find("span.red").length > 0 && typeof value == "undefined"){
				isvalid = false;
			}
		...
		$.ajax({
			...
			success	: function (data) {
				loadingHide();
				if (data.error == 1) {
					successMsg("问答保存成功！", 1000, function(){
						isOver = true;
					});
				} else if (data.error == -100) {
