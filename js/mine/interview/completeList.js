$(function(){
	...
	$(".reset-btn").on("click", function(){
		$("#search-status").val("").trigger('chosen:updated');
	...
	//订单详情查看
		$(".detail").on("click", function(){
			...
			openTabForParent(dataHref, "-interview-" + id, "实时面签-" + dataTitle);
		});
});
...
function exporIncomeExcel() {
	...
	confirmDialog("您确定要导出记录？", function () {
		...
		frame.attr("src", ctx + "/completeListReport/exportList.action?idArr=" + idArr.toString());
