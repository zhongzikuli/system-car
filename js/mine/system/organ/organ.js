$(document).ready(function () {
    ...
    function checkOrgName() {
        ...
        $.ajax({
            url: ctx + "/organ/checkOrgName.action",
            type: "post",
            ...
            success: function (data) {
                if (data.error == 1) {
                    ...
                    $("input[name='orgName']").addClass("input_validation-failed");
                }
            }
        });
    }

    $(".organFee").on('click',function(){
        var id=$(this).attr("data-id");
        var name=$(this).attr("data-name");
       var url=ctx+"/sysOrganFee/query.action?id="+id;
        openTabForParent(url, "-organFee-" , name+"-"+"费用管理" );
    });
})
// 新增单位
function createOrgan() {
    var options = {
        ...
        onBeforeShow: function () {
            $(".reset").trigger('click');
            $("#create-user-name").val(' ');//不能删除，见bug编号：253
            //城市组件
        ...
        callback: function () {
            ...
            if ($("#createOrganForm").valid("createOrganForm")) {
            	loadingShow();
                $.ajax({
                    url: ctx + "/organ/create.action",
                    type: "post",
                    data: $("#createOrganForm").serialize(),
                    dataType: 'json',
                    success: function (data) {
                    	loadingHide();
                        if (data.error == 1) {
    ...
    var options = {
        ...
        callback: function () {
            if ($("#editOrganForm").valid("editOrganForm")) {
            	loadingShow();
                $.ajax({
                    url: ctx + "/organ/update.action",
                    type: "post",
                    data: $("#editOrganForm").serialize(),
                    dataType: "json",
                    success: function (data) {
                        if (data.error == 1) {
                        	loadingHide();
...
function deleteOrgan() {
    ...
    if (ck.length == 0) {
        ...
        confirmDialog("确认删除选中的单位吗？", function () {
        	loadingShow();
            var params = {}
            params.idArr = idArr.toString();
            $.ajax({
                url: ctx + "/organ/delete.action",
                type: "post",
                data: params,
                dataType: "json",
                success: function (data) {
