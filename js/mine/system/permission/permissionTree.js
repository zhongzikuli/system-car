...
function initTree(id){
	$.ajax({
		url: ctx + "/permission/queryAllTree.action",
		type: "post",
		dataType:"json",
...
function createMenu(){
	...
	var options = {
		...
		callback:function()	{
			...
			var flag = true;
			$.ajax({
				url : ctx + "/permission/create.action",
				type : "post",
				data : obj,
				dataType: "json",
				async: false,
	...
	var options = {
		...
		callback:function()	{
			...
			$.ajax({
				...
				type : "post",
				data : {"id": id, "type": type, "parentIds": parentIds},
				dataType: "json",
				success : function(data) {
					if(data.error == 1){
						$('#permissionChangeFlag', window.parent.document).val(true);
...
function doSwapSortNo(node1, node2){
	$.ajax({
		url : ctx + "/permission/move.action",
		type : "post",
		data : {"id1": node1.id, "id2": node2.id, "sortNo1": node1.sortNo, "sortNo2": node2.sortNo},
		dataType: "json",
		success : function(data) {
			if(data.error == 1){
				$('#permissionChangeFlag', window.parent.document).val(true);
