	...
	if(checkValue){
		if(selectValue == submitValue){
			if($('#select2_span_' + selectId)[0]){
				$('#select2_span_' + selectId).addClass('input_validation-failed');
				$('#select2_span_' + selectId).attr('title', tipMsg);
			}
			return "faild";
		}else{
			if($('#select2_span_' + selectId)[0]){
				$('#select2_span_' + selectId).removeClass('input_validation-failed');
...
function removeRequire(lableId, tipMsg, objMsg, checkMsg){
	if(undefined != lableId && null != lableId){
		...
		$("#" + lableId).removeClass("input_validation-failed");
	}
}
