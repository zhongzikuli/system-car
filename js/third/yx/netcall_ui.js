...
(function() {
    ...
    NetcallBridge.fn.updateVideoShowSize = function(local, remote) {
        ...
        var smallSize = {
            width: this.isFullScreen ? 500 : 180,
            height: this.isFullScreen ? '100%' : 160
        };
        if (local) {
    ...
    NetcallBridge.fn.hideAllNetcallUI = function() {
        ...
        //@ TODO this.$audioShowBox.toggleClass("hide", true);
        this.$audioShowBox.toggleClass("hide", true);
        this.$beCallingBox.toggleClass("hide", true);
        this.$netcallBox.toggleClass("hide", true);
    ...
    NetcallBridge.fn.showConnectedUI = function(type) {
        ...
        //@ TODO this.$audioShowBox.toggleClass("hide", type !== Netcall.NETCALL_TYPE_AUDIO);
        var info = this.yx.cache.getUserById(this.netcallAccount);
        if (type === Netcall.NETCALL_TYPE_AUDIO) {

        	//@ TODO  this.$audioShowBox.find(".nick").text(this.yx.getNick(this.netcallAccount));
        	//@ TODO  this.$audioShowBox.find('img').attr('src', getAvatar(info.avatar));
    ...
    NetcallBridge.fn.showCallingUI = function(){
        ...
        this.$netcallBox.toggleClass("hide", false);
        var info = this.yx.cache.getUserById(this.netcallAccount);
    ...
    NetcallBridge.fn.updateBizVedioChannel = function(channelId) {
    	if(null == channelId){
    		return;
    	}else{
    		updateVedioChannel(channelId);
    	}
    };
})();