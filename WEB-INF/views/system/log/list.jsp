            ...
            <div class="table-responsive full-height">
                <table class="table table-hover table-height table-striped table-daily">
                    ...
                    <c:forEach var="item" items="${pageBean.recordList}" varStatus="st">
                        <tr>
	                        <td class="cel" style="max-width:100px;">${item.loginName}</td>
	                        <td class="cel" style="max-width:100px;"><sl:user userId="${item.creater}"/></td>
	                        <td class="cel" style="max-width:100px;">${item.sysPermissionName}</td>
	                        <td class="cel" style="max-width:150px;">${item.sysPermissionPercode}</td>
                            <td class="cel" style="max-width:150px;">${item.parentSysPermissonName}</td>
                            <td class="cel" style="max-width:200px;">${item.parentSysPermissonUrl}</td>
                            <td class="cel" style="max-width:80px;">${item.ip}</td>
                            <td class="cel" style="max-width:100px;">${item.type}</td>
	                        <td class="cel" style="max-width:140px;"><sl:format show="${item.ctime}" type="date" pattern="yyyy-MM-dd HH:mm "/></td>
                            <td class="btn-cel">
