...
<div class="mod_header">
    <div id="hiddenForm">
        ...
        <input type="hidden" name="carSerialFirst"  value="${paramMap.carSerialFirst}"/>
    </div>
    <form id="pagerForm" action="${ctx}/cfCarBrand/queryCarSerialBrandName.action" method="post" style="margin:0;">
            ...
            <div class="col-sm-10">
                <div class="form-inline">
                    <div class="form-group">
                        <label class="control-label label">车系(一级):</label>
                          <input type="hidden" name="brandNameMain"  id="brandNameMain" value="${paramMap.brandNameMain}"/>
                        <input type="text" name="carSerialFirst" class="form-control"
                               id="carSerialFirst" value="${paramMap.carSerialFirst}"/>
            ...
            <table class="table table-hover table-height table-striped">
                ...
                <c:forEach var="item" items="${pageBean.recordList}" varStatus="st">
                    <tr>
                        ...
                        <td >${item.carSerialFirst}</td>
                        <td class="btn-cel" style="padding-left: 250px;">
                            <shiro:hasPermission name="cfCarBrand:update">
                                <a href="#" class="btn btn-primary btn-xs edit" data-id="${item.carSerialFirst}">
                                    <i class="fa fa-edit"></i>编辑</a>
                            </shiro:hasPermission>

                            <a href="#"  class="btn btn-primary btn-xs start"  data-id="${item.carSerialFirst}"
                                    <c:if test="${item.forbidden ==0}">
                                        style="display:none;"
                                    </c:if>>
                                <i class="fa fa-edit"></i>启用</a>
                            <a href="#"  class="btn btn-danger btn-xs stop"  data-id="${item.carSerialFirst}"
                                    <c:if test="${item.forbidden ==1}">
                                        style="display:none;"
                                    </c:if>>
                                <i class="fa fa-edit"></i>禁用</a>

                            <a href="#"  class="btn btn-info btn-xs mainCar-Administration" data-id="${item.carSerialFirst}">
                                <i class="fa fa-cloud-download"></i>二级车系管理</a>
                        </td>
                    </tr>
                </c:forEach>
                </tbody>
...
<script type="text/javascript" src="${ctx}/js/mine/system/carBrand/carBrandFirstList.js"></script>
<script type="text/template" title="新增" id="carBrand-dialog">
    <div class="ibox-content">
        <form id="carBrandForm" class="form-horizontal">
            <div class="form-group">
                <div class="col-sm-12">
                    <label class="col-xs-3 control-label"><span class="red">*</span>车系(一级):</label>
                    <div class="col-xs-8">
                        <input type="text" class="form-control" name="carSerialFirst_add" id="carSerialFirst_add"
...
<script type="text/template" title="编辑" id="carBrand-dialog-edit">
    <div class="ibox-content" style="margin-bottom:0;">
        <form id="carBrandForm_edit" class="form-horizontal">
            <div class="form-group">
                <label class="col-xs-3 control-label"><span style="color:#ff2528;">*</span>车系(一级):</label>
                <div class="col-xs-8">
                    <input type="text" class="form-control" name="carSerialFirst_edit" id="carSerialFirst_edit"
                           check="cfCarBrandForm(this)">
                </div>
            </div>
        </form>
