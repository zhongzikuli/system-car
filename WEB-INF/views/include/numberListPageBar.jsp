					...
					<ul>
						<c:if test="${ pageBean.beginPageIndex > 1 }">
					   		...
					   		<c:choose>
					        	...
					        	<c:otherwise>
					   				<li class="o-ui-pager"><a href="javascript:void(0);" onclick="jumpPage(${ pageBean.currentPage-11})">上一页</a></li>
					        	</c:otherwise>
					        </c:choose>
					   	</c:if>
					   <c:choose>
					   				...
					   				<c:forEach var="i" begin="${ pageBean.beginPageIndex }" end="${ pageBean.endPageIndex }" step="1">
							        	<c:choose>
							        		<c:when test="${ i!= pageBean.currentPage }">
								   				<li class="o-ui-pager"><a href="javascript:void(0);" onclick="jumpPage(${ i })">${ i }</a></li>
							         	</c:when>
							         	<c:otherwise>
				        ...
				        <c:if test="${ pageBean.endPageIndex < pageBean.pageCount }">
				        	<c:choose>
				        		<c:when test="${ pageBean.currentPage +11 > pageBean.pageCount }">
						   				<li class="o-ui-pager"><a href="javascript:void(0);" onclick="jumpPage(${ pageBean.pageCount-5})">下一页</a></li>
					         	</c:when>
					         	<c:otherwise>
						   				<li class="o-ui-pager"><a href="javascript:void(0);" onclick="jumpPage(${ pageBean.currentPage+11 })">下一页</a></li>
					         	</c:otherwise>
				        	</c:choose>
					   				<li class="o-ui-pager"><a href="javascript:void(0);" onclick="jumpPage(${ pageBean.pageCount})">末页</a></li>
				        </c:if>
