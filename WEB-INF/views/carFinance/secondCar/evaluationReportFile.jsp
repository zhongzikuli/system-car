...
<div class="mod_header">
    <div id="hiddenForm">
        ...
        <input  type="hidden" name="status" value="${fileStatus}">
    ...
    <form id="pagerForm" action="${ctx}/evaluationReportFile/query.action" method="post" style="margin:0;">
            ...
            <div class="col-sm-7">
                ...
                <div class="btn-box animated fadeInRight">
                    <div class="row">
                        ...
                        <div class="col-sm-5">
                            <div class="form-group">
                                <label class="col-xs-3 control-label">提交状态:</label>
                                <div class="col-xs-8">
                                    <select class="form-control" id="repstatus" name="status">
                                        <option value="2" <c:if test="${fileStatus == '2'}">selected</c:if>>全部</option>
                                        <option value="0"  <c:if test="${fileStatus == '0'}">selected</c:if>>未提交</option>
                                        <option value="1" <c:if test="${fileStatus == '1'}">selected</c:if>>已提交</option>
                                    </select>
                                </div>
                            </div>
                            <div class="form-group">
            ...
            <div class="table-responsive full-height">
                <table class="table table-hover table-height table-striped">
                    ...
                    <tr>
                        ...
                        <th style="width: 6%">评估报告完成时间</th>
                        <th style="width: 6%">评估时间</th>
                        <th style="width: 6%">提交报告时间</th>
                    ...
                    <c:forEach var="item" items="${pageBean.recordList}" varStatus="st">
                        <tr>
                            ...
                            <td class="cel">
                                <c:if test="${item.fileStatus ==0}">未提交</c:if>
                                <c:if test="${item.fileStatus ==1}">已提交</c:if>
                            </td>
                            <td class="cel">
                            ...
                            <td class="cel">
                                <fmt:formatDate value="${item.evaluateReportFileApplyTime}" pattern="yyyy-MM-dd HH:mm "/>
                            </td>
                             <td class="btn-cel">
                                 <shiro:hasPermission name="uploadReport:uploadReport">
                                    <a title="报告状态" class="btn btn-success btn-xs"
                                    onclick="uploadReport('${item.businessOrderAcceptId}','${ctx}/evaluationReportFile/query.action')">
                                    <i class="fa fa-edit">上传评估报告</i></a>
                                 </shiro:hasPermission>
                            </td>
                        </tr>
                    </c:forEach>
...
<script type="text/javascript" src="${ctx}/js/mine/carFinance/secondCar/reportFile.js"></script>
</html>