...
<div class="mod_header">
    <div id="hiddenForm">
        ...
        <input type="hidden" name="bankLoan" value="${paramMap.bankLoan}">
        <input type="hidden" name="moneyPackageName" value="${paramMap.moneyPackageId}">
        
    </div>
    <form id="pagerForm" action="${ctx}/financialCushionLoanReport/queryForAdvancedLoanReport.action" method="post">
        <%@include file="/WEB-INF/views/include/pageForm.jsp" %>
        <div class="row">
         <div class="col-sm-11 text-left">
          	<div class="form-inline">
               <shiro:hasPermission name="financeStatements:exporLoanStatisticsList">
                ...
                <div class="btn-box animated fadeInRight">
                    <div class="row">
                        ...
                        <div class="form-group" >
                   	 	 ...
                   	 	 <div class="form-group">
                          <label class="col-xs-3 control-label">资金包:</label>
	                        <div class="col-xs-8">
	                          <select class="form-control" id="search-moneyPackage-name" name="moneyPackageName">
	                            <option value="">请选择</option>
	                            <c:forEach var="moneyPackage" items="${moneyPackage}" varStatus="dp">
	                                <c:choose>
	                                    <c:when test="${null == paramMap.moneyPackageId}">
	                                        <option value="${moneyPackage.id}">${moneyPackage.name}</option>
	                                    </c:when>
	                                    <c:when test="${paramMap.moneyPackageId == moneyPackage.id}">
	                                        <option selected="selected" value="${moneyPackage.id}">${moneyPackage.name}</option>
	                                    </c:when>
	                                    <c:otherwise>
	                                        <option value="${moneyPackage.id}">${moneyPackage.name}</option>
	                                    </c:otherwise>
	                    ...
	                    <div class="form-group">
		                        <label class="col-xs-3 control-label">快捷搜索:</label>
			                        <div class="col-xs-8">
				                        <input id="keyword" type="text" class="form-control" placeholder="请输入客户姓名或身份证号或上牌地或订单号" name="keyword" value="${paramMap.keyword}" onkeyup="value=value.replace(/\s/g,'')">
			                        </div>
                   	 	 </div> 
	                        ...
	                        <div class="col-xs-8">
                   	 	 ...
                   	 	 <div class="form-group">
		                       <label class="col-xs-3 control-label">银行放款:</label>
	                      	      <div class="col-xs-8">
			                           <select class="form-control" id="search-bank-loan" name="bankLoan">
			                            <option value="">请选择</option>
			                            <option value="1" <c:if test="${paramMap.bankLoan == 1}">selected</c:if>>是</option>
			                            <option value="0" <c:if test="${paramMap.bankLoan == 0}">selected</c:if>>否</option>
...
<script type="text/javascript" src="${ctx}/js/mine/carFinance/financialStatement/budgetStatisticsList.js"></script>
</html>
