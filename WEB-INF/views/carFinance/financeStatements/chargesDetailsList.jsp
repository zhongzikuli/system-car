...
<div class="mod_header">
    ...
    <form id="pagerForm" action="${ctx}/financialChargesDetailsReport/queryForChargesDetailsReport.action" method="post">
        <%@include file="/WEB-INF/views/include/pageForm.jsp" %>
        <div class="row">
         <div class="col-sm-11  text-left">
       	  <div class="form-inline">
			<shiro:hasPermission name="financeStatements:exporChargesDetailsList">
				<a type="button" class="btn btn-success btn-sm refresh-btn" onclick="exporChargesDetailsExcel()">导出</a>
			</shiro:hasPermission>
			<c:if test="${ not empty entity}">
                ...
                <%-- <div class="form-group">
                       <label class="alert-warning">费用合计(万元): </label>
                       <code class="alert-info"><sl:format type="number" show="${entity.serviceFee/10000+entity.channelEnsureMoney/10000+entity.licensePlateEnsureMoney/10000+entity.poundage/10000+entity.agreeEnsureMoney/10000}" pattern="#,##0.00"/></code>
                </div> --%>
	               ...
	               <a class="btn btn-primary  btn-sm btn-search">查询&nbsp;<i class="fa fa-caret-up"></i></a>
	                <div class="btn-box animated fadeInRight">
	                    ...
	                    <div class="col-sm-5">
		                    ...
		                    <div class="form-group">
										<label class="col-xs-3 control-label">到账确认:</label>
										<div class="col-xs-8">
											<select class="form-control" id="search-isArrival" name="isArrival">
												<option value="">请选择</option>
												<option value="1" <c:if test="${paramMap.isArrival == 1}"> selected="selected"</c:if> >是</option>
												<option value="0" <c:if test="${paramMap.isArrival == 0}"> selected="selected"</c:if> >否</option>
											</select>
										</div>
							</div>
		                    <%-- <div class="form-group">
                ...
                <table class="table table-hover table-height table-striped">
                    ...
                    <c:forEach var="item" items="${pageBean.recordList}" varStatus="st">
                        <tr>
                            ...
                            <td class="cel"><sl:format type="number" show="${item.serviceFee+item.channelEnsureMoney+item.licensePlateEnsureMoney+item.poundage+item.agreeEnsureMoney}"  pattern="#,##0.00"/></td>
