...
<div class="mod_header">
    ...
    <form id="pagerForm" action="${ctx}/financial/queryForExpenseAudit.action" method="post">
        <%@include file="/WEB-INF/views/include/pageForm.jsp" %>
        <div class="row">
        	<div class="col-sm-2">
        		<%-- <c:if test="${paramMap.orderStatus == 12}">
	        		<shiro:hasPermission name="financial:expenseAuditSeat">
	                          ...
	                          <label class="col-xs-3 control-label">财务垫付:</label>
		                        <div class="col-xs-8">
			                        <select class="form-control" id="search-style" name="orderStatus">
			                            ...
			                            <option value="0" <c:if test="${paramMap.orderStatus == 0}">selected</c:if>>全部</option>
	                            		<option value="12" <c:if test="${paramMap.orderStatus == 12}">selected</c:if>>未审核</option>
	                            		<option value="13" <c:if test="${paramMap.orderStatus == 13}">selected</c:if>>已审核</option>
			                        </select>
		                      </div>
	                    </div>
                    ...
                    <c:forEach var="item" items="${pageBean.recordList}" varStatus="st">
                        <tr>
                           ...
                           <%--  <c:if test="${paramMap.orderStatus == 12}">
                            ...
                            <td class="cel">
                                <fmt:formatDate value="${item.auditTimeDetail}" pattern="yyyy-MM-dd HH:mm"/>
                            </td>
                            <td class="btn-cel">
                            <c:if test="${item.orderStatus<13}">
	                            <shiro:hasPermission name="examinel:cost">
	                                    <a data-id="${item.id}" data-title="${item.realName}" data-from="${ctx}/cfBusinessOrderAccept/detail.action?id=${item.id}&active=financialPay"
	                                       class="btn btn-success btn-xs expenseAudit-detail"><i class="fa fa-search-plus"></i>费用确认</a>
                                </shiro:hasPermission>
                             </c:if>
...
<script type="text/javascript" src="${ctx}/js/mine/carFinance/finance/ExpenseAudit.js"></script>
</html>
