...
<div class="mod_header">
 <div id="hiddenForm">
    ...
    <form id="pagerForm" action="${ctx}/cfFileCenter/uploadInterview.action" method="post">
            ...
            <div class="col-sm-10">
                <div class="form-inline">
	                <div class="form-group">
	                    ...
	                    <label class="control-label label">面签资料状态:</label>
                ...
                <table class="table table-hover table-height table-striped">
                    ...
                    <c:forEach var="item" items="${pageBean.recordList}" varStatus="st">
                        <tr>
                            ...
                            <td class="cel">
                                <fmt:formatDate value="${item.uploadInterviewTime}" pattern="yyyy-MM-dd HH:mm"/>
                            </td>
                            <td class="cel">
                            ...
                            <c:if test="${(item.orderStatus==1 || item.orderStatus==3 || item.orderStatus==-2) && item.interviewFileStatus==0}">
                                <a title="上传面签资料" class="btn btn-primary btn-xs"
                                   onclick="uploadInterview('${item.id}','${ctx}/cfFileCenter/uploadInterview.action')">上传面签资料</a>
                            </c:if>
                            </shiro:hasPermission>
                               <shiro:hasPermission name="order:track">
                                    <a title="订单轨迹" class="btn btn-success btn-xs"
                                       onclick="toOrderTrack('${item.id}','${ctx}/cfFileCenter/uploadInterview.action')">订单轨迹</a>
                                </shiro:hasPermission>  
