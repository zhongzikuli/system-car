...
<div class="mod_header">
 <div id="hiddenForm">
    ...
    <form id="pagerForm" action="${ctx}/cfSupplyMaterial/carTransferFile.action" method="post">
            ...
            <div class="col-sm-10">
                <div class="form-inline">
	                <div class="form-group">
	                    <label class="control-label label">过户资料状态:</label>
                ...
                <table class="table table-hover table-height table-striped">
                    ...
                    <c:forEach var="item" items="${pageBean.recordList}" varStatus="st">
                        <tr>
                            ...
                            <td class="cel">
                                <c:if test="${item.carTransferFileStatus==0}"><code class="alert-default">待提交</code></c:if>
                                <c:if test="${item.carTransferFileStatus==1}"><code class="alert-info">已提交</code></c:if>
                                <c:if test="${item.carTransferFileStatus==2}"><code class="alert-success">通过</code></c:if>
                                <c:if test="${item.carTransferFileStatus==-2}"><code class="alert-warning">退回</code></c:if>
                            </td>                           
                             <td class="cel">
                            ...
                            <td class="cel">
                             ...
                             <td class="btn-cel" style="min-width:120px;">
                               <shiro:hasPermission name="carTransferFile:save">
                                <c:if test="${item.carTransferFileStatus==0||item.carTransferFileStatus==-2}">
                                    <a title="上传其他附件" class="btn btn-primary btn-xs"
                                       onclick="uploadOldCarFile('${item.id}','${ctx}/cfSupplyMaterial/carTransferFile.action')">过户资上传</a>
                                </c:if>
                               </shiro:hasPermission>
                                <shiro:hasPermission name="order:track">
                                 ...
                                 <a title="查看" class="btn btn-info btn-xs detail" href="${ctx}/cfBusinessOrderAccept/detail.action?id=${item.id}&goBackUrl=${ctx}/cfSupplyMaterial/carTransferFile.action">
