...
<div class="mod_header">
    ...
    <form id="pagerForm" action="${ctx}/carInfo/queryForAudit.action" method="post">
        <%@include file="/WEB-INF/views/include/pageForm.jsp" %>
        <div class="row">
        	<div class="col-sm-5">
        		<%--
                <shiro:hasPermission name="carSupply:change">
					<a class="btn btn-primary btn-sm change-audit-car-btn">换审核</a>
				</shiro:hasPermission>
        		 --%>
                <a class="btn btn-success btn-sm audit-car-refresh-btn">刷新</a>
            </div>
            <div class="col-sm-7">
                ...
                <div class="btn-box animated fadeInRight">
                    ...
                    <div class="col-sm-12">
                            <div class="form-group">
                                <label class="col-xs-2 control-label" style="margin-left: -14px;">提车资料上传时间:</label>
                                <div class="col-xs-9" style="width:82%;">
                                    <div class="input-group">
			                            <input type="text" class="form-control" name="startDate"
			                                   id="audit-car-search-start-date" value="${paramMap.startDate}"/>
			                            <span class="input-group-addon">到</span>
			                            <input type="text" class="form-control" name="endDate"
			                                   id="audit-car-search-end-date" value="${paramMap.endDate}"/>
			                        </div>
                                </div>
                            </div>
                        </div>
						...
						<c:if test="${userLevel == 2 or userLevel == 3}">
							<div class="form-group">
								...
								<div class="col-xs-8">
									<select class="form-control" id="search-select-car-department" name="depId">
							...
							<div class="form-group">
								...
								<div class="col-xs-8">
									<input id="keyword" type="text" class="form-control" placeholder="支持客户姓名、身份证号、订单编号" name="keyword" 
						...
						<div class="col-sm-5">
		                    <div class="form-group">
		                        ...
		                        <div class="col-xs-8">
			                        <select class="form-control" id="search-audit-car-status" name="auditFlag">
		                    ...
		                    <div class="form-group group-btn">
		                        <button type="button" class="btn  btn-sm btn-primary audit-car-search-btn" onclick="searchSubmit()">搜索</button>
		                        <a type="button" class="btn btn-info  btn-sm audit-car-reset-btn">重置</a>
		                    </div>
		                 </div>
		             </div>       
                </div>
            ...
            <div class="table-responsive full-height">
                <table class="table table-hover table-height table-striped">
                    ...
                    <tr>
                        ...
                        <th style="width:8%;">提车资料上传审核时间</th>
                        <th style="width:4%;">提车资料状态</th>
                        <th style="width:8%;">提车资料上传提交时间</th>
                    ...
                    <c:forEach var="item" items="${pageBean.recordList}" varStatus="st">
                        <tr>
							...
							<td class="cel used"><c:if test="${not empty item.operationingLoginNameContractPrint}"><code>${item.operationingLoginNameContractPrint}</code></c:if></td>
                            ...
                            <td>
                                <c:if test="${item.auditStatus==1 }">
                                	<shiro:hasPermission name="carSupply:audit">
	                                    <a data-id="${item.id}" data-order="${item.businessOrderAcceptId}" data-title="${item.realName }" class="btn btn-w-m btn-xs btn-success audit-car-btn">审核</a>
                                	</shiro:hasPermission>
                                </c:if>
...
<script type="text/javascript" src="${ctx}/js/mine/carFinance/audit/carList.js"></script>
</html>
