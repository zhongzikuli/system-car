...
<div class="mod_header">
    ...
    <form id="pagerForm" action="${ctx}/carInfo/queryForTransferAudit.action" method="post">
        <%@include file="/WEB-INF/views/include/pageForm.jsp" %>
        <div class="row">
        	<div class="col-sm-5">
                <shiro:hasPermission name="carTransfer:change">
					<a class="btn btn-primary btn-sm change-audit-transfer-btn">换审核</a>
				</shiro:hasPermission>
                <a class="btn btn-success btn-sm audit-transfer-refresh-btn">刷新</a>
            </div>
            <div class="col-sm-7">
                <a class="btn btn-primary  btn-sm btn-search">查询&nbsp;<i class="fa fa-caret-up"></i></a>
                <div class="ibox-content btn-box animated fadeInRight">
                    <div class="row mr-none">
                          ...
                          <div class="form-group pull-left">
                              ...
                              <div class="col-xs-8">
	                            ...
	                            <input type="text" class="form-control" name="startDate"
	                                   id="audit-transfer-search-start-date" value="${paramMap.startDate}"/>
	                            <span class="input-group-addon">到</span>
	                            <input type="text" class="form-control" name="endDate"
	                                   id="audit-transfer-search-end-date" value="${paramMap.endDate}"/>
	                        </div>
                              </div>
                          </div>
                         </div>
                         <div class="col-sm-4">
		                    <div class="form-group">
		                        ...
		                        <div class="col-xs-8">
			                        <select class="form-control" id="search-audit-transfer-status" name="auditFlag">
						...
						<div class="col-sm-4">
		                    <div class="form-group group-btn">
		                        <button type="button" class="btn  btn-sm btn-primary audit-transfer-search-btn" onclick="searchSubmit()">搜索</button>
		                        <a type="button" class="btn btn-info  btn-sm audit-transfer-reset-btn">重置</a>
		                    </div>
		                 </div>
		             </div>       
                </div>
                ...
                <table class="table table-hover table-height table-striped">
                    ...
                    <c:forEach var="item" items="${pageBean.recordList}" varStatus="st">
                        <tr>
							...
							<td class="cel used"><c:if test="${not empty item.operationingLoginName}"><code>${item.operationingLoginName}</code></c:if></td>
                             ...
                             <td>
                                <c:if test="${item.auditStatus == 1 }">
                                	<shiro:hasPermission name="carTransfer:audit">
	                                    <a data-id="${item.id}" data-order="${item.businessOrderAcceptId}" data-title="${item.realName }"  type="button" class="btn btn-w-m btn-xs btn-success audit-transfer-btn">审核</a>
                                	</shiro:hasPermission>
                                </c:if>
...
<script type="text/javascript" src="${ctx}/js/mine/carFinance/audit/transferList.js"></script>
</html>
